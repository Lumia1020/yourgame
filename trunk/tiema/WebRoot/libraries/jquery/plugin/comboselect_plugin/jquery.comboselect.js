...
(function($){
	jQuery.fn.initComboSelect = function(settings){
		settings = jQuery.extend({
			sort: 'both'		// which sides to sort: none, left, right, or both
		}, settings);
		
		var thiz = $(this);
		var csleft = thiz.find('.csleft');		//左下啦
		var csright = thiz.find('.csright');	//右下啦
		var cshidden = thiz.find('.cshidden');	//真是提交表单的select
		var theForm = thiz.parents('form');
		
		if(settings.submit){
			//提交表单前把右下啦的数据放到真是的下拉中去
			theForm.submit(function(){
				cshidden.empty();
				cshidden.append(csright.find('option').clone());
				cshidden.find('option').attr('selected','selected');
				return true;
			});		
		}
		...
		//左边列表双击
		csleft.dblclick(function(){
			thiz.find('.csadd').click();
		});
		
		//右边列表双击
		csright.dblclick(function(){
			thiz.find('.csremove').click();
		});
		
		//添加
		thiz.find('.csadd').click(function(){
			csright.append(csleft.find('option:selected'));
		});
		
		//添加所有
		thiz.find('.csaddall').click(function(){
			csright.append(csleft.find('option'));
			csleft.empty();
		});
		
		//删除
		thiz.find('.csremove').click(function(){
			csleft.append(csright.find('option:selected'));
		});
		
		//删除所有
		thiz.find('.csremoveall').click(function(){
			csleft.append(csright.find('option'));
			csright.empty();
		});
	};
	
	jQuery.fn.comboselect = function(settings){
		$(this).next('.comboselect').remove();
		...
		$('input.csadd').click(function(){
			...
			sortBoxes(left.attr('id'), right.attr('id'),settings);	
		});
	...
	function sortBoxes(leftID, rightID,settings){
		var toSort = null;
