...
function dTree(objName) {
	...
	this.icon = {
		root : 'libraries/dtree/images/system/menu/base.gif',
		folder : 'libraries/dtree/images/system/menu/folder.gif',
		folderOpen : 'libraries/dtree/images/system/menu/folderopen.gif',
		node : 'libraries/dtree/images/system/menu/page.gif',
		empty : 'libraries/dtree/images/system/menu/empty.gif',
		line : 'libraries/dtree/images/system/menu/line.gif',
		join : 'libraries/dtree/images/system/menu/join.gif',
		joinBottom : 'libraries/dtree/images/system/menu/joinbottom.gif',
		plus : 'libraries/dtree/images/system/menu/plus.gif',
		plusBottom : 'libraries/dtree/images/system/menu/plusbottom.gif',
		minus : 'libraries/dtree/images/system/menu/minus.gif',
		minusBottom : 'libraries/dtree/images/system/menu/minusbottom.gif',
		nlPlus : 'libraries/dtree/images/system/menu/nolines_plus.gif',
		nlMinus : 'libraries/dtree/images/system/menu/nolines_minus.gif'
	};
	this.obj = objName;
	this.aNodes = [];
	this.aIndent = [];
...
dTree.prototype.node = function(node, nodeId) {
	...
	if (node.pid == this.root.id) {
		...
		checkboxSyntax = "<input type='checkbox' desc='" + node.cshow + "' name='" + node.cname + "' id='" + node.cname + "_" + node.id + "' value='" + node.cvalue
				+ "' onClick='javascript: " + this.obj + ".checkNode(" + node.id + "," + node.pid + "," + node._hc + ",this.checked);' ";
		// 是否被选中
		if (node.cchecked)
	...
	if (this.root.id != node.pid) {
		...
		if (node._hc) {
			...
			if (!this.config.useLines)
				...
				str += ((node._io) ? ((node._ls && this.config.useLines) ? this.icon.minusBottom : this.icon.minus) : ((node._ls && this.config.useLines)
...
dTree.prototype.isHaveBNode = function(id, pid) {
	...
	for (var n = 0; n < this.aNodes.length; n++) {
		...
		if (this.aNodes[n].pid != -1 && this.aNodes[n].id != id && this.aNodes[n].pid == pid) {
			if($('#' + this.aNodes[n].cname + "_" + this.aNodes[n].id).attr('checked') == 'checked')
...
dTree.prototype.checkPNodeRecursion = function(pid, ischecked) {
	for (var n = 0; n < this.aNodes.length; n++) {
		if (this.aNodes[n].pid != -1 && this.aNodes[n].id == pid) {
			$('#' + this.aNodes[n].cname + "_" + this.aNodes[n].id).attr('checked',ischecked);
...
dTree.prototype.nodeStatus = function(status, id, bottom) {
	...
	eJoin.src = (this.config.useLines) ? ((status) ? ((bottom) ? this.icon.minusBottom : this.icon.minus) : ((bottom) ? this.icon.plusBottom : this.icon.plus)) : ((status)
			? this.icon.nlMinus
			: this.icon.nlPlus);
