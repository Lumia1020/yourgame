package com.tiema.core.dao.hibernate.core;
...
/**
 * A copy of Hibernate's Example class, with modifications that allow you to include many-to-one and one-to-one
 * associations in Query By Example (QBE) queries.
 * 
 * @author original code by Gavin King, modified by jkelly, enhanced again by George Wei
...
public class EnhancedExample implements Criterion {
	...
	private static final long serialVersionUID = -1616235395528397347L;
	...
	static final class NotEmptyPropertySelector implements PropertySelector, Serializable {
		@Override
		public boolean include(Object propertyValue, String propertyName, Type type) {
	...
	public EnhancedExample excludeEmpty() {
	...
	public static EnhancedExample create(Object entity) {
		...
		return new EnhancedExample(entity, NOT_NULL, true);
	}
	...
	public static EnhancedExample createDefault(Object entity) {
		if (entity == null)
			throw new NullPointerException("null EnhancedExample");
		return new EnhancedExample(entity, NOT_NULL, false).enableLike(MatchMode.ANYWHERE).ignoreCase().excludeZeroes();
	}

	public static EnhancedExample createDefault(Object entity, boolean isAndRelations) {
		if (entity == null)
			throw new NullPointerException("null EnhancedExample");
		return new EnhancedExample(entity, NOT_NULL, isAndRelations).enableLike(MatchMode.ANYWHERE).ignoreCase().excludeZeroes();
	}

	/**
	 * @Title: createDefaultAll
	 * @Description: 实力查询,默认匹配模糊查询和忽略大小写
	 * @param entity 需要查询的示例
	 * @param isAndRelations 是否是用 and 来匹配所有属性之间的关系
	 */
	public static EnhancedExample createDefaultAll(Object entity, boolean isAndRelations) {
		if (entity == null)
			throw new NullPointerException("null EnhancedExample");
		return new EnhancedExample(entity, NOT_EMPTY, isAndRelations).enableLike(MatchMode.ANYWHERE).ignoreCase();
	}
	...
	private boolean isPropertyIncluded(Object value, String name, Type type) {
		return !excludedProperties.contains(name) && selector.include(value, name, type)
				&& (!type.isAssociationType() || (type.isAssociationType() && includeAssociations && !type.isCollectionType()));
	}
