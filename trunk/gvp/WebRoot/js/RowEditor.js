...
Ext.ux.grid.RowEditor = Ext.extend(Ext.Panel, {
    ...
    startEditing: function(rowIndex, doFocus){
        ...
        if(this.fireEvent('beforeedit', this, rowIndex) !== false){
            ...
            var cm = g.getColumnModel(), fields = this.items.items, f, val,hiddenValue,displayValue;
            for(var i = 0, len = cm.getColumnCount(); i < len; i++){
                val = this.preEditValue(record, cm.getDataIndex(i));
                f = fields[i];
                if(f.getXType() == 'combo'){
                	displayValue = this.preEditValue(record,f.displayField);
                	
                	f.setValue(displayValue);
                	
                	if(f.hiddenField){
                		f.hiddenField.value = val;
                	}
                }else{
                	f.setValue(val);
                }
    ...
    stopEditing : function(saveChanges){
        ...
        if(saveChanges === false || !this.isValid()){
    		...
    		if(record){
	    		if(!record.isValid()){
	    			this.fireEvent('invalid',record,this.grid.getStore());
	    		}
    		}
            return;
        }
    ...
    onRender: function(){
        ...
        this.bwrap.up('div.x-row-editor').setStyle('z-index','8000'); 
    },
