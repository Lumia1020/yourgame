$(function() {
	...
	// 创建分页组件,创建完毕后执行分页查询
	createPagination('#page_infos', [15, 20, 10, 5], findAllGolfClubs, true);
	 ...
	 */
	function findAllGolfClubs(wrap_id, page_no, init, callback) {
	...
	function renderDataTr(n, b) {
		...
		return "<tr id='tr_" + n.id + "' class='i M " + (b ? 'B' : '') + "'><td class='cx'><input type='radio' name='radio' sid='" + n.id
				...
				+ n.clubName + "</td><td class='" + cs + "'>" + $.t(n.contact) + "</td><td class='" + cs + "'>" + $.t(n.telephone)
				+ "</td><td class='" + cs + "'>" + $.t(n.fax) + "</td><td class='" + cs + "'>" + $.t(n.address) + "</td><td class='" + cs + "'>"
	...
	//验证添加表单
	addEntity('#form_add','#table_list','golfClub',renderDataTr,changeToolbarButtonState);
	addEntity('#form_club_price_add',$('#table_list'),'clubPrice',renderDataDetailsTr,changeToolbarButtonStateByDetailsRow,renderDetailsHead);
	
	//验证修改表单
	updateEntity('#form_update','golfClub',"#tr_",renderDataTr,changeToolbarButtonState);
	updateEntity('#form_club_price_update','clubPrice',"#tr_details_",renderDataDetailsTr,changeToolbarButtonStateByDetailsRow);
	
	//删除俱乐部价格记录
	deleteEntity('#btn_delete_price_state',$('#table_list'),'input[name="radio_details"]:checked','#tr_details_','json/club-price/success!delete','clubPriceId',changeToolbarButtonStateByDetailsRow,renderNoDetailsDataTr);
		...
		if (id) {
			if (confirm('确定要' + btn_text + '吗？')) {
				$.ajax({
					type : "POST",
					dataType : "json",
					url : 'json/golf-club/success!update',
					data : {'golfClub.id':id,'golfClub.state':state},
	...
	$('#btn_show_price_add').click(function(){
		var id = $('input[name="radio"]:checked','#table_list').attr('sid');
		if(id){
			$('#clubPrice_golfClub_id_add').val(id);
			tb_show('新增俱乐部价格', '#TB_inline?height=310&width=700&inlineId=div_club_price_add', false);
		}else{
			alert('请先选择要操作的俱乐部记录!');
		}
	});
	...
	$('#btn_show_add').click(function() {
		tb_show('新增俱乐部资料', '#TB_inline?height=380&width=700&inlineId=div_add', false);
	});
	...
	$('#btn_query').click(function() {
		tb_remove();
		findAllGolfClubs('#page_infos', 0, false);
	});
	...
	$('#btn_show_update').click(function() {
		...
		if (id) {
			...
			$.ajax({
				type : "POST",
				url : 'json/golf-club/entity-load',
				dataType : "json",
				data : {"golfClubId" : id},
				success : function(json) {
					if (json.success) {
						var obj = json.golfClub;
						for (field in obj) {$("#form_update [name='golfClub." + field + "']").val(obj[field]);}
					}
					$.waiting('#btn_update', false, '更新');
				}
			});
			$.clearValidationErrors($('#form_update'));
			tb_show('修改俱乐部资料', '#TB_inline?height=380&width=700&inlineId=div_update', false);
		} else {
			alert('请先选择要操作的行');
		}
	});
	...
	$('#btn_show_price_update').click(function(){
		...
		if (id) {
			...
			tb_show('修改俱乐部价格', '#TB_inline?height=310&width=700&inlineId=div_club_price_update', false);
		} else {
			alert('请先选择要操作的行');
		}
	});
	...
	function changeToolbarButtonState() {
		...
		switch(state){
			case "VALID":
				$('#btn_change_state').val('取消俱乐部');
				break;
			case "INVALID":
				$('#btn_change_state').val('激活俱乐部');
				break;
		}
	}
