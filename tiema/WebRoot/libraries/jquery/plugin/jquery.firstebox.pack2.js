...
function fb_show(caption, url, imageGroup,compact) {
	parentDocument = this.frameElement.ownerDocument;
	parentBody = parentDocument.body;
    try {
        ftop = 0,
        fleft = 0,
        fftop = 0,
...
//        };
        ...
        if(typeof parentBody.style.maxHeight === 'undefined'){
        	$(parentBody).css({
        		height:'100%',
        		width:'100%'
        	});
        	$('html',$(parentDocument)).css('overflow','hidden');
        	 if (parentDocument.getElementById("FB_HideSelect") === null) {
                $(parentBody).append("<iframe id='FB_HideSelect'></iframe><div id='FB_overlay'  class='TB_overlayBG2'></div><div id='FB_window' class='bd_upload'></div>");
                $("#FB_overlay",$(parentBody)).click(fb_remove);
            }
        } else {
            if (parentDocument.getElementById("FB_overlay") === null) {
                $(parentBody).append("<div id='FB_overlay' class='TB_overlayBG2'></div><div id='FB_window' class='bd_upload'></div>");
                $("#FB_overlay",$(parentBody)).click(fb_remove);
            }
        };
        
        if (fb_detectMacXFF()) {
            $("#FB_overlay",$(parentBody)).addClass("FB_overlayMacFFBGHack");
        } else {
            $("#FB_overlay",$(parentBody)).addClass("FB_overlayBG");
        };
...
//        $("body").append("<div id='FB_load'><img src='" + imgLoader.src + "' /></div>");
        $(parentBody).append("<div id='FB_load'><img src='" + imgLoader.src + "' /></div>");
        $('#FB_load').show();
        var baseURL;
        if (url.indexOf("?") !== -1) {
...
//                $("#FB_closeWindowButton").click(fb_remove);
                $("#FB_window",$(parentBody)).append("<img id='FB_Image' src='" + url + "' width='" + imageWidth + "' height='" + imageHeight + "' alt='" + caption + "'/><div id='hoverNav'>" + FB_PrevHTML1 + FB_NextHTML1 + "</div><div id='FB_caption'>" + caption + "<div id='FB_secondLine'>" + FB_imageCount + FB_PrevHTML + FB_NextHTML + "</div></div><div id='FB_closeWindow'><a href='#' id='FB_closeWindowButton' title='Close'><img src='/swap/images/ico_closetip.png' border='0'/></a></div>");
                $("#FB_closeWindowButton",$(parentBody)).click(fb_remove);
                if (!(FB_PrevHTML === "")) {
                    function goPrev() {
...
//                        $("body").append("<div id='FB_window' class='bd_upload'></div>");
                        if ($(parentDocument).unbind("click", goPrev)) {
                            $(parentDocument).unbind("click", goPrev);
                        };
                        $("#FB_window",$(parentBody)).remove();
                        $(parentBody).append("<div id='FB_window' class='bd_upload'></div>");
...
//                    $("#prevLink").click(goPrev);
                    $('#prevLink',$(parentBody)).height(imageHeight);
                    $("#FB_prev",$(parentBody)).click(goPrev);
                    $("#prevLink",$(parentBody)).click(goPrev);
                };
                if (!(FB_NextHTML === "")) {
                    function goNext() {
//                        $("#FB_window").remove();
//                        $("body").append("<div id='FB_window'></div>");
                        $("#FB_window",$(parentBody)).remove();
                        $(parentBody).append("<div id='FB_window'></div>");
...
//                    $("#nextLink").click(goNext);
                    $("#FB_next",$(parentBody)).click(goNext);
                    $('#nextLink',$(parentBody)).height(imageHeight);
                    $("#nextLink",$(parentBody)).click(goNext);
                };
...
//                    }
//                };
                parentDocument.onkeydown = function (e) {
                    if (e == null) {
                        keycode = event.keyCode;
                    } else {
                        keycode = e.which;
...
//                    display: "block"
//                });
                $("#FB_load",$(parentBody)).remove();
                $("#FB_ImageOff",$(parentBody)).click(fb_remove);
                $("#FB_window",$(parentBody)).css({
                    display: "block"
                });
            };
            imgPreloader.src = url;
//            $("#FB_window").fdrag(true);
            $("#FB_window",$(parentBody)).fdrag(true);
        } else {
...
//                fwidth = $("body").width() * fwidth;
                fwidth = $(parentBody).width() * fwidth;
            };
            if (fheight <= 1) {
//                fheight = document.documentElement.clientHeight * fheight;
                fheight = parentDocument.documentElement.clientHeight * fheight;
            };
...
//                }
                $("#FB_iframeContent",$(parentBody)).remove();
                if (params['modal'] != "true") {
                    $("#FB_window",$(parentBody)).append("<div id='FB_title' class='tipstitle2'><div id='FB_ajaxWindowTitle'>" + caption + "</div><div id='FB_closeAjaxWindow'><a href='#' id='FB_closeWindowButton' title='Close' style='background:none repeat scroll 0 0 #DC4835;border:1px solid #A7190F;'><img src='/swap/images/ico_closetip.png' border='0'/></a></div></div><iframe frameborder='0' hspace='0' src='" + urlNoQuery[0] + "' id='FB_iframeContent' name='FB_iframeContent" + Math.round(Math.random() * 1000) + "' onload='fb_showIframe()' style='width:" + (ajaxContentW + 29) + "px;height:" + (ajaxContentH + 17) + "px;' > </iframe>");
                } else {
                    $("#FB_overlay",$(parentBody)).unbind();
                    $("#FB_window",$(parentBody)).append("<iframe frameborder='0' hspace='0' src='" + urlNoQuery[0] + "' id='FB_iframeContent' name='FB_iframeContent" + Math.round(Math.random() * 1000) + "' onload='fb_showIframe()' style='width:" + (ajaxContentW + 29) + "px;height:" + (ajaxContentH + 17) + "px;'> </iframe>");
                }
            } else {
...
//                }
                if ($("#FB_window",$(parentBody)).css("display") != "block") {
                    if (params['modal'] != "true") {
                    	if(($.browser.msie&&($.browser.version == "6.0")&&!$.support.style) || !compact){
                        	$("#FB_window",$(parentBody)).append("<div id='FB_title' class='tipstitle2'><div id='FB_ajaxWindowTitle'>" + caption + "</div><div id='FB_closeAjaxWindow'><a href='#' id='FB_closeWindowButton' title='Close'><img src='/swap/images/ico_closetip.png' border='0'/></a></div></div><div id='FB_ajaxContent' style='width:" + ajaxContentW + "px;height:" + ajaxContentH + "px'></div>");
                    	}else{
                        	$("#FB_window",$(parentBody)).append("<div id='FB_title' class='tipstitle2'><div id='FB_ajaxWindowTitle'>" + caption + "</div><div id='FB_closeAjaxWindow'><a href='#' id='FB_closeWindowButton' title='Close'><img src='/swap/images/ico_closetip.png' border='0'/></a></div></div><div id='FB_ajaxContent' ></div>");
                    	}
                    } else {
                        $("#FB_overlay",$(parentBody)).unbind();
                        if(($.browser.msie&&($.browser.version == "6.0")&&!$.support.style) || !compact){
                        	$("#FB_window",$(parentBody)).append("<div id='FB_ajaxContent' class='FB_modal' style='width:" + ajaxContentW + "px;height:" + ajaxContentH + "px;'></div>");
                        }else{
                        	$("#FB_window",$(parentBody)).append("<div id='FB_ajaxContent' class='FB_modal'></div>");
                        }
                    }
                } else {
                	if(($.browser.msie&&($.browser.version == "6.0")&&!$.support.style) || !compact){
	                    $("#FB_ajaxContent",$(parentBody))[0].style.width = ajaxContentW + "px";
	                    $("#FB_ajaxContent",$(parentBody))[0].style.height = ajaxContentH + "px";
                	}
                    $("#FB_ajaxContent",$(parentBody))[0].scrollTop = 0;
                    $("#FB_ajaxWindowTitle",$(parentBody)).html(caption);
                }
            };
//            $("#FB_closeWindowButton").click(fb_remove);
...
//            }
            $("#FB_closeWindowButton",$(parentBody)).click(fb_remove);
            if (url.indexOf('FB_inline') != -1) {
                $("#FB_ajaxContent",$(parentBody)).append($('#' + params['inlineId']).children());
                $("#FB_window",$(parentBody)).unload(function () {
                    $('#' + params['inlineId'],$(parentBody)).append($("#FB_ajaxContent",$(parentBody)).children());
                });
                fb_position(compact);
                $("#FB_load",$(parentBody)).remove();
                $("#FB_window",$(parentBody)).fadeIn('fast');
            } else if (url.indexOf('FB_iframe') != -1) {
                fb_position(compact);
                if ($.browser.safari) {
                    $("#FB_load",$(parentBody)).remove();
                ...
                $("#FB_ajaxContent",$(parentBody)).load(url += "&random=" + (new Date().getTime()), function () {
                    fb_position(compact);
                    $("#FB_load",$(parentBody)).remove();
                    fb_init("#FB_ajaxContent a.firstebox");
                    $("#FB_window",$(parentBody)).fadeIn('fast');
...
//                }
//            };
            $("#FB_window",$(parentBody)).setHandler('FB_title',(params['move'] == 'false'));
            parentDocument.onkeyup = function (e) {
                if (e == null) {
                    keycode = event.keyCode;
                } else {
                    keycode = e.which;
...
function fb_remove() {
    $(".formError",$(parentBody)).hide();
    $("#FB_imageOff",$(parentBody)).unbind("click");
    $("#FB_closeWindowButton",$(parentBody)).unbind("click");
    $("#FB_window",$(parentBody)).fadeOut("fast", function () {
        $('#FB_window,#FB_overlay,#FB_HideSelect',$(parentBody)).trigger("unload").unbind().remove();
    });
    $("#FB_load",$(parentBody)).remove();
    if (typeof parentDocument.body.style.maxHeight == "undefined") {
        $(parentBody).css({
            height: "auto",
            width: "auto"
        });
        $("html",$(parentDocument)).css("overflow", "");
    };
    parentDocument.onkeydown = "";
    parentDocument.onkeyup = "";
    return false;
};
...
function fb_position(compact) {
    ...
    $("#FB_window",$(parentBody)).fPosition({
        vpos: "middle",
        hpos: "center",
        fw: FB_WIDTH,
        fh: FB_HEIGHT
...
function fb_getPageSize() {
    var de = parentDocument.documentElement;
    var w = window.innerWidth || self.innerWidth || (de && de.clientWidth) || parentDocument.body.clientWidth;
    var h = window.innerHeight || self.innerHeight || (de && de.clientHeight) || parentDocument.body.clientHeight;
    arrayPageSize = [w, h];
...
(function ($) {
    $.fn.fPosition = function (options) {
        ...
        return this.each(function (index) {
            ...
            if (jQuery.browser.opera) {
                ...
                $this.css("top", ($(parentDocument).scrollTop() + (parseInt(window.innerHeight) / 2) - (options.fh / 2)) + "px");
            } else {
    ...
    $.getMousePosition = function (e) {
        ...
        if (e.pageX || e.pageY) {
            ...
            posx = e.clientX + parentDocument.body.scrollLeft + parentDocument.documentElement.scrollLeft;
            posy = e.clientY + parentDocument.body.scrollTop + parentDocument.documentElement.scrollTop;
        };
        return {
            'x': posx,
            'y': posy
    ...
    $(parentDocument).mousemove(function (e) {
    ...
    $(parentDocument).mouseup(function (e) {
        if (isMouseDown && dragStatus[currentElement.id] != 'false') {
            isMouseDown = false;
...
$(window).scroll(function () {
    ...
    $("#FB_window",$(parentBody)).css("top", (ftop + ftop1 + $(parentDocument).scrollTop()) + "px").css("left", (fleft + $(parentDocument).scrollLeft()) + "px");
});